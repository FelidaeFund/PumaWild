...
for(i = 0; i < OCDOQOCDCC.roadObjects.Length; i++){
comboValues.Add(OCDOQOCDCC.roadObjects[i]);
}
...
OCDOQOCDCC.ODDOOOCODD.Add(thisso);
}
}
}

OCDOQOCDCC.roadObjects = comboValues.ToArray();
OCDOQOCDCC.OQQQQODCOC();
...
for(int i = 0 ; i < OCDOQOCDCC.ODDOOOCODD.Count;i++){
ODODDQQO so = (ODODDQQO)OCDOQOCDCC.ODDOOOCODD[i];
